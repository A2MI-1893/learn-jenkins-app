pipeline {
    ...
    stages {
        ...
        stage('AWS') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'JENKINS_AWS', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                    sh '''
                        aws --version
                        echo "Hello S3!" > index.html
                        aws s3 cp index.html s3://learn-jenkins-202410151035/index.html
                    '''
                }
            }
        }
        ...
        stage('Tests') {
            parallel {
                ...
                stage('E2E') {
                    ...
                    post {
                        always {
                            publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'playwright-report', reportFiles: 'index.html', reportName: 'Playwright local testing Report', reportTitles: '', useWrapperFileDirectly: true])
                        }
                    }
                }
            }
        ...
        /* stage('Deploy prod') {
            ...
            environment {
                CI_ENVIRONMENT_URL = 'https://nimble-taffy-74457c.netlify.app'
            }
